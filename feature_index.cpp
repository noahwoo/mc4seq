...
int EncoderFeatureIndex::getID(const char *key) {
  ...
  if (it == dic_.end()) {
    ...
    int n = maxid_;
    // each unigram feature expand to #tag features
    // each bigram feature expand to #tag*#tag features
    maxid_ += (key[0] == 'U' ? y_.size() : y_.size() * y_.size());
    return n;
  } else {
	// increment the feature occurrence count
    it->second.second++;
    // return the found id
