...
class TaggerImpl : public Tagger {
 ...
 public:
  ...
  const int *emission_vector(size_t i, size_t j) const {
	/* std::cout << "node_.size=" << node_.size() << ",ysize=" << 
		node_[0].size() << ", i=" << i << ",j=" << j << std::endl; */
