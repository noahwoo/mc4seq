...
bool TaggerImpl::add2(size_t size, const char **column, bool copy) {
  ...
  if (mode_ == LEARN) { // check if answer was put in last column
    size_t r = ysize_;
...
bool TaggerImpl::read(std::istream *is) {
  ...
  for (;;) { // read each line of node for current sample x, until reach empty line or blank line
...
void TaggerImpl::buildLattice() {
  ...
  // build the node-path-node lattice (two tags candidate)
  //  T11 - T21 - T31
  //         X       X
  //  T12 - T22 - T32
...
void TaggerImpl::viterbi() {
  ...
  double bestc = -1e37;
  // search for the starting point at the end
  Node *best = 0;
  size_t s = x_.size()-1;
  for (size_t j = 0; j < ysize_; ++j) {
  ...
  // tracing back to resolve the best prediction tag
  for (Node *n = best; n; n = n->prev)
    result_[n->x] = n->y;

  // the cost related to the best path
...
double TaggerImpl::gradient(double *expected) {
  ...
  // subtract the gradient related to the correct tag by 1:
  // g = \sum_{n=1}^N F(x_i, y_i) - E_{P(y|x_i,w)} F(x_i, y)
  for (size_t i = 0;   i < x_.size(); ++i) {
