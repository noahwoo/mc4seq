...
// get feature of a node by current template, row should satisfy |row| <= 4
...
bool FeatureIndex::buildFeatures(TaggerImpl *tagger) {
  ...
  // build the unigram features, starts from first node
  for (size_t cur = 0; cur < tagger->size(); ++cur) {
  ...
  // build the bigram features, starts from the second node
  for (size_t cur = 1; cur < tagger->size(); ++cur) {
